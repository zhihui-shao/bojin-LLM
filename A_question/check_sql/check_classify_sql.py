    ...
    for obj in datas:
        ...
        try:
            ...
            if cursor.rowcount > 0:
                output_file = "A_right_sql.json"  # 每次循环迭代都将结果追加到JSON文件，全对
                with open(output_file, "a", encoding="utf-8") as output:
                    json.dump({"a_id": id, "a_question": question, "sql": sql}, output, ensure_ascii=False, indent=4)
                    output.write(',' + '\n')
            else:
                output_file = "A_half_sql.json"  # 每次循环迭代都将结果追加到JSON文件，可以执行但是结果为0
                with open(output_file, "a", encoding="utf-8") as output:
                    json.dump({"a_id": id, "a_question": question, "sql": sql}, output, ensure_ascii=False, indent=4)
                    output.write(',' + '\n')
        except Exception as e:
            output_file = "A_wrong_sql.json"  # 每次循环迭代都将结果追加到JSON文件，无法执行
            with open(output_file, "a", encoding="utf-8") as output:
                json.dump({"a_id": id, "a_question": question, "sql": sql}, output, ensure_ascii=False, indent=4)
                output.write(',' + '\n')
